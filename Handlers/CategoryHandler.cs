...
//manipular as informacoes que serao persistidas no banco
public class CategoryHandler(AppDbContext context) : ICategoryHandler
{
    ...
    public async Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
    {
        ...
        {
            return new Response<Category?>(null, 500, "Não possível obter a categoria");
        }
    }
    ...
    public async Task<PagedResponse<List<Category>>> GetAllAsync(GetAllCategoriesRequest request)
    {
        try
        {
            //algoritmo que filtra as categorias de um usuario logado/autenticado
            var query = context
                .Categories
                .AsNoTracking()
            ...
            //algoritmo que filtra de maneira paginada as categorias de um usuario
            var categories = await query
                .Skip(request.PageSize * (request.PageNumber - 1))
                .Take(request.PageSize)
                .ToListAsync();
            
            //conta o número de categorias de um usuario
